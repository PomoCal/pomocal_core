...
class BookAPIManager: ObservableObject {
    ...
    private var clientID: String? {
        UserDefaults.standard.string(forKey: "NaverClientID")
            ?? ProcessInfo.processInfo.environment["NAVER_CLIENT_ID"]
    }

    private var clientSecret: String? {
        UserDefaults.standard.string(forKey: "NaverClientSecret")
            ?? ProcessInfo.processInfo.environment["NAVER_CLIENT_SECRET"]
    }
    ...
    func searchBooks(query: String) async {
        ...
        guard let clientID, let clientSecret, !clientID.isEmpty, !clientSecret.isEmpty else {
            await MainActor.run {
                ...
                self.errorMessage = "Naver API credentials are not configured."
            }
            return
        }
